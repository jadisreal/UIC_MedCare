        ...
        <div className="flex h-screen bg-gray-100">
            ...
            <div className={`flex-1 flex flex-col transition-all duration-300 ease-in-out ${isSidebarOpen ? 'ml-64' : 'ml-20'}`}>
                ...
                <main className="flex-1 p-6 overflow-y-auto bg-white">
                    ...
                    {showSuccess && (
                        <div className="fixed top-20 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-50">Consultation Scheduled!</div>
                    )}
                    ...
                    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            <div>
                                ...
                                <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#A3386C] text-black" required />
                            </div>
                            <div>
                                <label className="block text-black text-sm font-bold mb-2">Time *</label>
                                <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#A3386C] text-black" required />
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end">
                            <button type="submit" className="bg-[#A3386C] text-white px-6 py-3 rounded-lg hover:bg-[#77536A] font-semibold">Schedule</button>
                        </div>
                    </form>
                </main>
            </div>
